...
sudo docker build . -f base.dockerfile --no-cache -t yperuvemba/cs380dproject:base --network=host
sudo docker push yperuvemba/cs380dproject:base

sudo docker build . -f client.dockerfile --no-cache -t yperuvemba/cs380dproject:client --network=host
sudo docker push yperuvemba/cs380dproject:client

sudo docker build . -f frontend.dockerfile --no-cache -t yperuvemba/cs380dproject:frontend --network=host
